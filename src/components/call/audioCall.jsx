        ...
        const unsub = onValue(callRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                if (data.status === 'ringing' && data.from !== username && !callState.isActiveCall) {
                    setCallState({ isIncomingCall: true, isOutgoingCall: false, isActiveCall: false, callWith: data.from, callType: 'audio', callId: data.callId });
                }
            ...
            <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl p-8 w-full max-w-md mx-auto shadow-2xl`}>
                ...
                <div className="flex justify-center space-x-6">
                    {callState.isIncomingCall && !callState.isActiveCall ? (
                        <>
                            <button onClick={handleAcceptCall} className="p-4 bg-green-500 hover:bg-green-600 rounded-full transition-colors"><FaPhone className="text-white text-xl" /></button>
                            <button onClick={handleRejectCall} className="p-4 bg-red-500 hover:bg-red-600 rounded-full transition-colors"><FaPhoneSlash className="text-white text-xl" /></button>
                        </>
                    ) : (
                        <button onClick={handleEndCall} className="p-4 bg-red-500 hover:bg-red-600 rounded-full transition-colors"><FaPhoneSlash className="text-white text-xl" /></button>
                    )}
                </div>
