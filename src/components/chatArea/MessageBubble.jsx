...
export default function MessageBubble({ msg, index, username, getProfilePhoto, onOpenMedia, activeChatType }) {
    ...
    return (
        <div className={`max-w-[85%] md:max-w-[70%] break-all h-auto min-w-[15%] p-3 rounded-2xl shadow-sm ${isOwnMessage ? 'ml-auto bg-[#00a884] text-white' : 'mr-auto bg-white text-gray-800 border border-gray-200'}`}>
            ...
            {msg.type === 'image' ? (
                <div className="my-1">
                    ...
                    {msg.fileName && <p className={`text-xs mt-1 ${isOwnMessage ? 'text-gray-200' : 'text-gray-500'}`}>{msg.fileName}</p>}
                </div>
            ) : msg.type === 'video' ? (
                <div className="my-1">
                ...
                <div className="my-1">
                    <div className="flex items-center gap-3 p-3 bg-gray-100 rounded-2xl border border-gray-300 hover:bg-gray-200 transition-colors cursor-pointer group" onClick={() => window.open(msg.message, '_blank', 'noopener,noreferrer')} title="Click to open document">
                        <div className="shrink-0">{getFileIcon(msg.fileName, msg.format)}</div>
                        <div className="flex-1 min-w-0">
                            <p className={`text-sm font-medium truncate ${isOwnMessage ? 'text-white' : 'text-gray-800'}`}>{msg.fileName || 'Document'}</p>
                            <p className={`text-xs ${isOwnMessage ? 'text-gray-200' : 'text-gray-600'}`}>{getFileTypeName(msg.fileName, msg.format)}</p>
                        </div>
                        <div className="shrink-0 flex gap-1">
                            <button onClick={(e) => { e.stopPropagation(); const link = document.createElement('a'); link.href = msg.message; link.target = '_blank'; link.rel = 'noopener noreferrer'; document.body.appendChild(link); link.click(); document.body.removeChild(link); }} className={`p-2 rounded-full transition-colors ${isOwnMessage ? 'text-gray-200 hover:text-white hover:bg-[#008f70]' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-300'}`} title="Download file">⬇️</button>
                            <div className={`p-2 transition-colors ${isOwnMessage ? 'text-gray-200 group-hover:text-white' : 'text-gray-400 group-hover:text-[#00a884]'}`}>🔗</div>
                        </div>
                    </div>
                </div>
            ) : (
                <p className={`text-left whitespace-pre-wrap break-words ${isOwnMessage ? 'text-white' : 'text-gray-800'}`}>{msg.message}</p>
            )}

            <p className={`text-xs mt-2 text-right ${isOwnMessage ? 'text-gray-200' : 'text-gray-500'}`}>{msg.time}</p>
        </div>
    );
}