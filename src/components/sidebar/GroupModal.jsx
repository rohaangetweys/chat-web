        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg p-4 w-full max-w-sm mx-auto shadow-lg`}>
                ...
                <div className="flex gap-2 justify-end">
                    <button onClick={closeGroupModal} className={`cursor-pointer px-3 py-1 text-sm ${isDark ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} rounded transition-colors font-medium`}>Cancel</button>
                    <button onClick={handleCreateGroup} disabled={!groupName.trim() || selectedUsers.length === 0} className="cursor-pointer px-3 py-1 text-sm bg-[#0084ff] text-white rounded hover:bg-[#0084ff] transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium">Create</button>
                </div>
            </div>
        </div>
    );
